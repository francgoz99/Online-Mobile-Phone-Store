...
product__box--border-hover text-center float-left float-4">
                                <div class="product__img-box">
                                    ...
                                    <?php if($_SESSION['msmsuid']==""){?>
                                        <a href="login.php" class="btn btn--box btn--small
btn--gray btn--uppercase btn--weight
btn--hover-zoom product__upper-btn">Add to Cart</a>
                                    <?php } else {?>
                                        <form method="post">
                                            <input type="hidden" name="pid" value="<?php echo $row['ID'];?>">
                                            <button type="submit" name="submit" class="btn
btn--box btn--small btn--gray btn--uppercase
btn--weight btn--hover-zoom product__upper-btn">Add to Cart</button>
                                        </form>
                                    <?php }?>
                ...
                <div class="page-pagination" align="center">
                    ...
                    <ul class="page-pagination__list" >
                        <li class="page-pagination__item"><a href="?pageno=1"
                                                             class="page-pagination__link btn btn--gray" >First</a></li>
                        <li class="<?php if($pageno <= 1){ echo 'disabled'; } ?> page-pagination__item">
                            <a href="<?php if($pageno <= 1)
                            { echo '#'; } else { echo "?pageno=".($pageno - 1); } ?>"
                               class="page-pagination__link btn btn--gray" >Prev</a>
                        </li>

                        <li class="<?php if($pageno >= $total_pages)
                        { echo 'disabled'; } ?> page-pagination__item">
                            <a href="<?php if($pageno >= $total_pages)
                            { echo '#'; } else { echo "?pageno=".($pageno + 1); } ?>"
                               class="page-pagination__link btn btn--gray" >Next</a>
                        </li>
