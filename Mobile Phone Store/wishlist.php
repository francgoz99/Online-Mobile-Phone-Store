...
if (strlen($_SESSION['msmsuid'])){
    ...
    if (isset($_POST['submit'])){
        ...
        if ($query){
            echo "<script>alert('Mobile has been added to cart');</script>";
...
?>
    ...
    <html lang="zxx">
    <head>
        <title>Online Mobile Phone Store||Wishlist Page</title>
                    ...
                    <div class="table-content table-responsive cart-table-content m-t-30">
                        <table>
                            ...
                            <tr>
                                <?php
                                $userid=$_SESSION['msmsuid'];
                                $query=mysqli_query($con, "select tblproducts.Image1,tblproducts.ProductName,tblproduct.BrandName,tblproducts.ModelNumber,tblproducts.Price,tblproducts.Stock,tblwish.ProductId,tblwish.ID from tblwish join tblproducts on 
                          tblproducts.ID=tblwish.ProductId where tblwish.UserId='$userid'");
                                $num=mysqli_num_rows($query);
                                if ($num>0){
                                    while ($row=mysqli_fetch_array($query)){
                                        ?>
                                <td class="product-thumbnail"><a href="single.php?pid=<?php echo $row['ID'];?>"><img class="img-fluid" src="admin/images/<?php echo $row['Image1'];?>" width="150" height="150" alt=""></a></td>
                                <td class="product-name"><a href="single.php?pid=<?php echo $row['ID'];?>"><?php echo $row['ProductName'];?></a></td>
                                <td class="product-price-cart"><span class="amount">$<?php $row['Price'];?></span></td>
                                <td class="product-subtotal">$<?php echo $row['Price'];?></td>
                                <td class="product-remove"><a href="wishlist.php?delid=<?php echo $row['ID'];?>" onclick="return confirm('Do you really want to Delete?');"><i class="fa fa-times"></i></a></td>
                                <td class="product-add-cart"><?php if ($_SESSION['msmsuid==""']){ ?><a href="login.php" class="btn btn--box btn--small btn--blue btn--uppercase btn--weight">Add to Cart</a>
                                   <?php } else{?>
                                <form method="post">
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            ...
                            <div class="col-md-6 modal__border">
                                ...
                                <div>
                                    <a href="#" class="btn--box btn--large btn--blue btn--uppercase btn--weight" data-dismiss="modal">Continue Shopping</a>
